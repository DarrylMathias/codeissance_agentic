...
app.post("/api/citypulse", async (req, res) => {
  const { prompt, startLat, startLong, endLat, endLong } = req.body;
  console.log(`SERVER: Prompt: "${prompt}"`);
  console.log(`SERVER: Origin Location: Latitude=${startLat}, Longitude=${startLong}`);
  console.log(`SERVER: Origin Location: Latitude=${endLat}, Longitude=${endLong}`);

  if (!prompt && !startLat && !startLong && !endLat && !endLong) {
    return res.status(400).json({ error: "The 'prompt', 'latitude', and 'longitude' fields are required." });
  }

  try {
    const agentResponse = await runMultiToolAgent(prompt, startLat, startLong, endLat, endLong);
