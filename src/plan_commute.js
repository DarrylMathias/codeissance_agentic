...
async function runCommuteAgent() {
  ...
  const prompt = ChatPromptTemplate.fromMessages([
    ["system", "You are an expert Mumbai route and transport analyst. Your task is to use the 'getMumbaiCommuteOptions' tool to retrieve hardcoded travel data (Andheri East to Nariman Point on Saturday 9 AM). You must execute the tool without asking for user input, then analyze the results, clearly stating the distance, and the estimated duration for each mode (Driving, Transit, Walking, Bicycling). Finally, highlight the **fastest** overall option."],
    ["human", "Analyze the predefined commute options for Mumbai and provide a detailed, easy-to-read summary."],
    ["placeholder", "{agent_scratchpad}"],
  ]);

  const agent = await createToolCallingAgent({ llm: model, tools: commuteTools, prompt });
